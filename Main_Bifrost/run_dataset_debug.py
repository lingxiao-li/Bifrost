...
dataset1 = YoutubeVOSDataset(**DConf.Train.YoutubeVOS) # work 
dataset2 = SaliencyDataset(**DConf.Train.Saliency) # work
...
dataset6 = SAMDataset(**DConf.Train.SAM) # work
# dataset7 = UVODataset(**DConf.Train.UVO.train)
dataset8 = VitonHDDataset(**DConf.Train.VitonHD) # work
# dataset9 = UVOValDataset(**DConf.Train.UVO.val)
dataset10 = MoseDataset(**DConf.Train.Mose) # work
# dataset11 = FashionTryonDataset(**DConf.Train.FashionTryon)
dataset12 = LvisDataset(**DConf.Train.Lvis) # work
...
def vis_sample(item):
    ...
    print(ref.shape, depth.shape, tar.shape, hint.shape, step.shape)
    ...
    vis = cv2.hconcat([ref.astype(np.float32), depth.astype(np.float32), hint_image.astype(np.float32), hint_mask.astype(np.float32), tar.astype(np.float32) ])
