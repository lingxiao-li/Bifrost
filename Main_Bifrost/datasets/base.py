...
class BaseDataset(Dataset):
    ...
    def process_pairs(self, ref_image, ref_mask, tar_image, tar_mask, tar_depth, max_ratio = 0.8):
        ...
        cv2.normalize(tar_depth, tar_depth_norm, alpha=0, beta=255,
        ...
        cropped_depth = tar_depth_norm[y1:y2, x1:x2]
        ...
        cropped_depth = pad_to_square(cropped_depth, pad_value = 0, random = False).astype(np.uint8)
        collage = pad_to_square(collage, pad_value = 0, random = False).astype(np.uint8)
        collage_mask = pad_to_square(collage_mask, pad_value = 2, random = False).astype(np.uint8)
        H2, W2 = collage.shape[0], collage.shape[1]

        cropped_target_image = cv2.resize(cropped_target_image.astype(np.uint8), (512,512)).astype(np.float32)
        cropped_depth = cv2.resize(cropped_depth.astype(np.uint8), (512, 512)).astype(np.uint8)
        H, W, C = cropped_depth.shape
        cropped_depth = cv2.resize(cropped_depth, (W, H), interpolation=cv2.INTER_LINEAR)
        ...
        cropped_depth = cropped_depth / 255
