...
class SaliencyDataset(BaseDataset):
    ...
    def get_sample(self, idx):
        ...
        if 'DUTS' in image_path:
            depth_path = image_path.replace(
                '.jpg', '.png').replace('Image', 'Depth')
        elif 'HFlickr' in image_path:
            depth_path = image_path.replace(
                '.jpg', '.png').replace('real_images', 'real_depth')
        elif 'MSRA' in image_path:
            depth_path = image_path.replace(
                '.jpg', '.png').replace('Imgs', 'Depth')
        ...
        tar_image = ref_image
        # =====================
        tar_depth = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)

        ref_mask = instances_mask
        tar_mask = instances_mask
        item_with_collage = self.process_pairs(
            ref_image, ref_mask, tar_image, tar_mask, tar_depth)
